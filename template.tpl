...

{
  ...
  "description": "Tag that sends events to Braze from Snowplow, GAv4 or other clients. Works best with the Snowplow Client.",
  "containerContexts": [
    "SERVER"
  ]
}
...
[
  {
    "type": "TEXT",
    "name": "apiEndpoint",
    "displayName": "Braze REST API Endpoint",
        ...
        "subParams": [
          ...
          },
          {
            ...
            "simpleTableColumns": [
              ...
              {
                "defaultValue": "",
                "displayName": "Braze Mapped Name (optional)",
                "name": "mappedKey",
                "type": "TEXT",
                "isUnique": true,
                "valueHint": "The key name which is sent to Braze",
                "valueValidators": []
              },
              {
        ...
        "subParams": [
          {
            ...
            "defaultValue": true,
            "alwaysInSummary": true,
            "help": "Include the event properties from the common event definition in Braze event object."
          },
          {
            "type": "SIMPLE_TABLE",
            ...
            "simpleTableColumns": [
              ...
              {
                "defaultValue": "",
                "displayName": "Braze Mapped Key (optional)",
                "name": "mappedKey",
                "type": "TEXT"
              }
            ],
        ...
        "subParams": [
          {
            ...
            "defaultValue": true,
            "alwaysInSummary": true,
            "help": "Include the user_data properties from the common event definition as Braze user attributes."
          },
          {
            "type": "SIMPLE_TABLE",
...
const mkUserAttributesObject = (evData, tagConfig) => {
  const attributes = {
    language: evData.language,
    country: evData['x-sp-geo_country'],
    time_zone: evData['x-sp-geo_timezone'],
  };

  if (evData['x-sp-geo_longitude'] && evData['x-sp-geo_latitude']) {
    attributes.current_location = {
      longitude: evData['x-sp-geo_longitude'],
      latitude: evData['x-sp-geo_latitude'],
    };
  }

  if (tagConfig.includeCommonUserProperties && evData.user_data) {
    attributes.email = evData.user_data.email_address;
    attributes.phone = evData.user_data.phone_number;
    attributes.address = evData.user_data.address;
  }

  if (tagConfig.userMappingRules && tagConfig.userMappingRules.length > 0) {
    return merge([attributes, getEventDataByKeys(tagConfig.userMappingRules)]);
  }
  return attributes;
};
...
const mkEventProperties = (evData, tagConfig) => {
  ...
  if (tagConfig.includeCommonEventProperties) {
    properties.page_location = evData.page_location;
    properties.page_encoding = evData.page_encoding;
    properties.page_referrer = evData.page_referrer;
    properties.page_title = evData.page_title;
    properties.screen_resolution = evData.screen_resolution;
    properties.viewport_size = evData.viewport_size;
  }

  if (tagConfig.eventMappingRules && tagConfig.eventMappingRules.length > 0) {
    return merge([properties, getEventDataByKeys(tagConfig.eventMappingRules)]);
  }
  return properties;
};
...
const getEventTime = (tagConfig) => {
  if (tagConfig.timeProp) {
    return getEventData(tagConfig.timeProp);
  }
...
const mkBrazePayload = (evData, tagConfig) => {
  ...
  const eventObject = mkEventObject(evData, tagConfig);
...
const mkRequestOptions = (tagConfig) => {
  const authHeader = 'Bearer ' + tagConfig.apiKey;
  return {
    method: 'POST',
    timeout: 5000,
    headers: {
...
// Main
const url = data.apiEndpoint + '/users/track';
const eventData = getAllEventData();

const requestOptions = mkRequestOptions(data);
const requestPayload = mkBrazePayload(eventData, data);
...
- name: Test default config
  code: |-
    ...
    mock('getTimestampMillis', function() {
      return 1642397654056;  // '2022-01-17T05:34:14.056Z'
    });
    const testTime = '2022-01-17T05:34:14.056Z';
    ...
    assertThat(argOptions.headers.Authorization).isStrictlyEqualTo('Bearer ' + mockData.apiKey);
  ...
  code: |-
    ...
    const testEvent = {
      ...
      "x-sp-contexts_com_snowplowanalytics_snowplow_ua_parser_context_1":[
        {"useragentFamily":"IE","useragentMajor":"7","useragentMinor":"0","useragentPatch":null,"useragentVersion":"IE 7.0","osFamily":"Windows XP","osMajor":null,"osMinor":null,"osPatch":null,"osPatchMinor":null,"osVersion":"Windows XP","deviceFamily":"Other"}
      ],
...
- name: Test Snowplow settings 2
  code: |-
    ...
    const testEvent = {
      ...
      "language":"en-US",
      "page_encoding":"UTF-8",
      "page_hostname":"test.example.com",
      "page_location":"http://test.example.com",
      "page_path":"/",
      "screen_resolution":"1920x1080",
      "user_id":"tester",
      "viewport_size":"1023x980",
      "user_agent":"curl/7.74.0",
      "host":"engineering-sandbox.appspot.com",
      "x-sp-app_id":"testApp",
      "x-sp-platform":"srv",
      "x-sp-dvce_created_tstamp":"1634551546989",
      "x-sp-name_tracker":"sptest",
      "x-sp-v_tracker":"curl",
      "x-sp-domain_sessionidx":1,
      "x-sp-br_colordepth":"24",
      "x-sp-br_viewwidth":1023,
      "x-sp-br_viewheight":980,
      "x-sp-dvce_screenwidth":1920,
      "x-sp-dvce_screenheight":1080,
      "x-sp-doc_charset":"UTF-8",
      "x-sp-doc_width":1023,
      "x-sp-doc_height":980,
      "x-sp-tp2":{"e":"ue","p":"srv","tna":"sptest","aid":"testApp","cs":"UTF-8","lang":"en-US","res":"1920x1080","cd":"24","tz":"Europe/London","dtm":"1634551546989","vp":"1023x980","ds":"1023x980","vid":"1","uid":"tester","url":"http://test.example.com","tv":"curl","ue_pr":"{\"schema\": \"iglu:com.snowplowanalytics.snowplow/unstruct_event/jsonschema/1-0-0\",\"data\":{\"schema\":\"iglu:com.acme.test/foo/jsonschema/1-0-0\",\"data\":{\"foo_age\":10}}}","co":"{\"schema\":\"iglu:com.snowplowanalytics.snowplow/contexts/jsonschema/1-0-0\",\"data\":[{\"schema\":\"iglu:com.snowplowanalytics.snowplow/mobile_context/jsonschema/1-0-2\",\"data\":{\"osType\":\"myOsType\",\"osVersion\":\"myOsVersion\",\"deviceManufacturer\":\"myDevMan\",\"deviceModel\":\"myDevModel\"}},{\"schema\":\"iglu:com.snowplowanalytics.snowplow/web_page/jsonschema/1-0-0\",\"data\":{\"id\":\"c000a256-1e13-4cde-a5b4-d7a00cf88003\"}}]}"},
      "x-sp-self_describing_event_com_acme_test_foo_1":{
        "foo_age":10
      },
      "x-sp-contexts_com_snowplowanalytics_snowplow_mobile_context_1":[
        {"osType":"myOsType","osVersion":"myOsVersion","deviceManufacturer":"myDevMan","deviceModel":"myDevModel"}
      ],
      "x-sp-contexts_com_snowplowanalytics_snowplow_web_page_1":[
        {"id":"c000a256-1e13-4cde-a5b4-d7a00cf88003"}
      ],
      "ga_session_number":"1",
      "x-ga-mp2-seg":"1",
      "x-ga-protocol_version":"2",
      "x-ga-page_id":"c000a256-1e13-4cde-a5b4-d7a00cf88003",
      "ip_override":"1.2.3.4"
    };
...
- name: Test event property rules
  code: |-
    const mockData = {
      ...
      "eventMappingRules":[
        {"key":"ip_override","mappedKey":"ip_address"}
      ],
      "includeCommonUserProperties":true,
    };
    ...
    const expectedBody = {
      "attributes":[
        {
          "country":"US",
          "current_location":{"longitude":-122.4124,"latitude":37.443604},
          "external_id":"jon.doe"
        }
      ],
      "events":[
...
- name: Test user property rules
  code: |-
    const mockData = {
      ...
      "userMappingRules":[
        {
          "key":"x-sp-contexts_com_snowplowanalytics_snowplow_mobile_context_1.0.deviceManufacturer",
          "mappedKey":"device"
        },
        {
          "key":"x-sp-self_describing_event_com_acme_test_foo_1.foo_age",
          "mappedKey":"user_age"
        }
      ],
    };
...
Created on 12/27/2021, 2:18:32 PM
