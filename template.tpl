...
// Constants
const tagName = 'Braze';
const brazeApiPath = '/users/track';
...
const requestOptions = mkRequestOptions(data, false);
const requestPayload = mkBrazePayload(eventData, data);

if (loggingEnabled) {
  const logRequestOptions = mkRequestOptions(data, true);
...
- name: Test entity rules - include all - edit - sde at root
  code: |
    ...
    // Test also logType: 'no' does not log on debug
    assertApi('logToConsole').wasNotCalled();
...
- name: Test event property rules - sde at root
  code: |
    ...
    // Test also logType: 'always' does log on debug
    assertApi('logToConsole').wasCalled();
...
- name: Test user property rules
  code: |
    ...
    // Test also logType: 'always' does log on prod
    assertApi('logToConsole').wasCalled();
...
- name: Test logs - Request and Response
  code: |
    ...
    // Assert on Request and Response Logs
    const expectedRequestLog = jsonApi.stringify({
      ...
      EventName: 'page_view',
      RequestMethod: 'POST',
      RequestUrl: mockData.apiEndpoint + '/users/track',
      RequestHeaders: {
        'Content-Type': 'application/json',
